...
export const publicProjectsRoutes: FastifyPluginAsync = async (fastify) => {
  fastify.get('/', async (request, reply) => {
    ...
    // Only return projects that are not drafts/archived AND are PUBLIC
    const conditions = and(
      inArray(projects.status, ['RECRUITING', 'ACTIVE', 'COMPLETED']),
  ...
  fastify.get('/:slug', async (request, reply) => {
    ...
    if (!project || project.status === 'DRAFT' || project.status === 'ARCHIVED' || project.visibility !== 'PUBLIC') {
